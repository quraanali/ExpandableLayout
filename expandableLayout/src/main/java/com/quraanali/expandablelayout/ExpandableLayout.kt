...
class ExpandableLayout @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) :
    ...
    init {
        if (attrs != null) {
            ...
            duration = a.getInt(R.styleable.ExpandableLayout_elDuration, DEFAULT_DURATION)
            expansion = if (a.getBoolean(
                    R.styleable.ExpandableLayout_elExpanded,
                    false
                )
            ) 1f else 0f
            orientation = a.getInt(R.styleable.ExpandableLayout_android_orientation, VERTICAL)
            parallax = a.getFloat(R.styleable.ExpandableLayout_elParallax, 1f)
            a.recycle()
